...
enum lttng_rotation_status lttng_rotate_session_get_status(
		...
		char **path)
{
	...
	*path = zmalloc(PATH_MAX);
	...
	if (rotate_handle->status == LTTNG_ROTATION_STATUS_COMPLETED) {
		...
		struct lttng_rotate_session_handle *rotate_handle)
{
	...
	if (ret < 0) {
		rotate_handle->status = LTTNG_ROTATION_STATUS_ERROR;
		ret = -1;
		goto end;
	}
...
int lttng_rotate_get_timer(const char *session_name,
		uint64_t *rotate_timer)
{
	struct lttcomm_session_msg lsm;
	struct lttng_rotate_get_timer *get_timer = NULL;
	int ret;

	memset(&lsm, 0, sizeof(lsm));
	lsm.cmd_type = LTTNG_ROTATE_GET_TIMER;
	lttng_ctl_copy_string(lsm.session.name, session_name,
			sizeof(lsm.session.name));
...
int lttng_rotate_get_size(const char *session_name,
		uint64_t *rotate_size)
{
	struct lttcomm_session_msg lsm;
	struct lttng_rotate_get_size *get_size = NULL;
	int ret;

	memset(&lsm, 0, sizeof(lsm));
	lsm.cmd_type = LTTNG_ROTATE_GET_SIZE;
	lttng_ctl_copy_string(lsm.session.name, session_name,
			sizeof(lsm.session.name));
